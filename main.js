...
var getUserColor = (clientId) => {
  const colors = ["#FF5555", "#55FF55", "#5555FF", "#FFAA55", "#55AAFF", "#AA55FF"];
...
function debounce(func, wait) {
  ...
  return (...args2) => {
    ...
    timeout = setTimeout(() => func(...args2), wait);
  };
}
var RelayPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.yDocs = /* @__PURE__ */ new Map();
    this.providers = /* @__PURE__ */ new Map();
    this.updateTimeouts = /* @__PURE__ */ new Map();
    this.cursorElements = /* @__PURE__ */ new Map();
    this.cursorUpdateIntervals = /* @__PURE__ */ new Map();
  }
  async onload() {
    ...
    this.registerView("relay-cursor-view", (leaf) => new LiveCursorView(leaf, this));
    this.addCommand({
      id: "show-cursor-view",
      name: "Show live cursors",
      callback: () => this.activateCursorView()
    });
    this.addCommand({
    ...
    this.registerEvent(
      // @ts-ignore: 'editor-paste' is a valid event not recognized by TypeScript
      this.app.workspace.on("editor-paste", async (evt, editor, view) => {
        if (!view.file || !this.settings.sharedFolders.some((folder) => view.file.path.startsWith(folder)))
          return;
        ...
        if (files && files.length > 0) {
          ...
          for (const file of Array.from(files)) {
            if (file.type.startsWith("image/")) {
              await this.handlePastedImage(file, view);
            }
          }
        }
      })
  ...
  async initializeSync() {
    const { serverUrl, accessKey, sharedFolders } = this.settings;
    for (const folder of sharedFolders) {
      ...
      yFiles.observe((event) => {
        event.keysChanged.forEach(async (filePath) => {
          ...
          const normalizedPath = (0, import_obsidian.normalizePath)(`${folder}/${filePath}`);
          if (typeof content === "string") {
            this.debounceWrite(normalizedPath, content);
            await this.checkMarkdownImages(content, folder, yDoc);
          } else if (content === void 0) {
            try {
              await this.app.vault.adapter.remove(normalizedPath);
              new import_obsidian.Notice(`File ${filePath} deleted`);
            } catch {
              new import_obsidian.Notice(`Failed to delete file ${filePath}`);
            }
          }
        });
      });
      this.app.vault.on("modify", (file) => {
        if (file instanceof import_obsidian.TFile && file.path.startsWith(folder) && file.extension === "md") {
          this.app.vault.adapter.read(file.path).then((content) => {
            const relativePath = file.path.replace(`${folder}/`, "");
            if (yFiles.get(relativePath) !== content) {
      ...
      this.app.vault.on("delete", (file) => {
        if (file instanceof import_obsidian.TFile && file.path.startsWith(folder)) {
          const relativePath = file.path.replace(`${folder}/`, "");
        ...
        event.keysChanged.forEach(async (filename) => {
          ...
          if (attachment) {
            ...
            try {
              ...
              new import_obsidian.Notice(`Attachment ${filename} deleted`);
            } catch {
              new import_obsidian.Notice(`Failed to delete attachment ${filename}`);
            }
          }
        });
      });
      const debouncedUpdateCursors = debounce((folder2, provider2) => {
        this.updateCursors(folder2, provider2);
      }, 1e3);
      const localClientId = provider.awareness.clientID;
      this.registerEvent(
        this.app.workspace.on("editor-change", (editor, ctx) => {
          if (ctx instanceof import_obsidian.MarkdownView && ctx.file?.path.startsWith(folder)) {
            const cursor = editor.getCursor();
            const relativePath = ctx.file.path.replace(`${folder}/`, "");
            provider.awareness.setLocalStateField("cursor", {
              file: relativePath,
              line: cursor.line,
              column: cursor.ch,
              userId: localClientId,
              nickname: this.settings.nickname || `User ${localClientId}`
            });
      ...
      provider.awareness.on("update", () => {
        console.log("Awareness states:", Array.from(provider.awareness.getStates().entries()).map(([id2, state]) => ({
          id: id2,
          nickname: state.cursor?.nickname,
          file: state.cursor?.file,
          line: state.cursor?.line,
          column: state.cursor?.column,
          isLocal: id2 === provider.awareness.clientID
        })));
        debouncedUpdateCursors(folder, provider);
      });
      this.registerDomEvent(document, "input", (event) => {
        ...
        if (activeView && activeView.file?.path.startsWith(folder) && target.closest(".cm-editor")) {
          const editor = activeView.editor;
          const cursor = editor.getCursor();
          const relativePath = activeView.file.path.replace(`${folder}/`, "");
          provider.awareness.setLocalStateField("cursor", {
            file: relativePath,
            line: cursor.line,
            column: cursor.ch,
          ...
          this.updateCursors(folder, provider);
        }
      });
      const intervalId = setInterval(() => {
        this.updateCursors(folder, provider);
      }, 1e3);
      this.cursorUpdateIntervals.set(folder, intervalId);
      debouncedUpdateCursors(folder, provider);
    }
  }
  async initializeFolderFiles(folder, yDoc) {
    ...
    if (folderObj instanceof import_obsidian.TFolder) {
      const yFiles = yDoc.getMap("files");
      const yAttachments = yDoc.getMap("attachments");
      const processFiles = async (folder2) => {
        for (const child of folder2.children) {
          if (child instanceof import_obsidian.TFile) {
            const relativePath = child.path.replace(`${folder2.path}/`, "");
      ...
      await processFiles(folderObj);
    }
  }
  debounceWrite(filePath, content) {
    if (this.updateTimeouts.has(filePath)) {
    ...
    this.updateTimeouts.set(
      ...
      setTimeout(async () => {
        try {
          const currentContent = await this.app.vault.adapter.read(filePath).catch(() => "");
          if (currentContent !== content) {
            await this.app.vault.adapter.write(filePath, content);
          }
        } catch {
          new import_obsidian.Notice(`Failed to write to ${filePath}`);
        }
  ...
  async uploadAttachment() {
    ...
    input.onchange = async () => {
      ...
      await this.uploadFile(file, this.settings.sharedFolders[0]);
    };
    input.click();
  }
  ...
  async handlePastedImage(file, view) {
  ...
  async activateCursorView() {
    const leaf = await this.app.workspace.getLeaf();
    await leaf.setViewState({ type: "relay-cursor-view", active: true });
  }
  async loadSettings() {
  ...
  onunload() {
    this.cursorElements.forEach((elements) => elements.forEach((el) => el.remove()));
    this.cursorElements.clear();
    this.providers.forEach((p) => p.destroy());
    this.yDocs.forEach((d) => d.destroy());
    this.updateTimeouts.forEach((t) => clearTimeout(t));
    this.cursorUpdateIntervals.forEach((t) => clearInterval(t));
    this.cursorUpdateIntervals.clear();
  }
  updateCursors(folder, provider) {
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView);
    if (!activeView || !activeView.file || !activeView.file.path.startsWith(folder)) {
      this.clearCursors(folder);
      return;
    }
    ...
    const cmLines = editorEl.querySelectorAll(".cm-line");
    this.clearCursors(folder);
    const newElements = [];
    states.forEach((state, clientId) => {
      if (state.cursor && state.cursor.file === relativePath && clientId !== provider.awareness.clientID) {
        try {
          const { line, column, nickname } = state.cursor;
          if (line < cmLines.length) {
            const lineEl = cmLines[line];
            let charWidth = 8;
            const computedStyle = getComputedStyle(lineEl);
            const fontSize = parseFloat(computedStyle.fontSize) || 16;
            charWidth = fontSize * 0.6;
            const spanEl = lineEl.querySelector("span");
            if (spanEl) {
    ...
    this.cursorElements.set(folder, newElements);
  }
  updateCursorsForClient(folder, provider, clientId) {
    ...
    const cmLines = editorEl.querySelectorAll(".cm-line");
    this.clearCursors(folder);
    if (state?.cursor && state.cursor.file === relativePath) {
      const { line, column, nickname } = state.cursor;
      if (line < cmLines.length) {
        const lineEl = cmLines[line];
        let charWidth = 8;
  ...
  clearCursors(folder) {
    const elements = this.cursorElements.get(folder) || [];
    elements.forEach((el) => el.remove());
    this.cursorElements.set(folder, []);
  }
};
var LiveCursorView = class extends import_obsidian.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.plugin = plugin;
  }
  getViewType() {
    return "relay-cursor-view";
  }
  getDisplayText() {
    return "Live Cursors";
  }
...
var RelaySettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Server URL").setDesc("URL of the Relay server").addText(
      (text) => text.setValue(this.plugin.settings.serverUrl).onChange(async (value) => {
        this.plugin.settings.serverUrl = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Access Key").setDesc("Key for server authentication").addText(
      (text) => text.setValue(this.plugin.settings.accessKey).onChange(async (value) => {
        this.plugin.settings.accessKey = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Shared Folders").setDesc("Comma-separated list of folders to sync").addText(
      (text) => text.setValue(this.plugin.settings.sharedFolders.join(",")).onChange(async (value) => {
        this.plugin.settings.sharedFolders = value.split(",").map((f) => f.trim());
        await this.plugin.saveSettings();
        await this.plugin.initializeSync();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Nickname").setDesc("Your display name for cursor labels").addText(
      (text) => text.setValue(this.plugin.settings.nickname).onChange(async (value) => {
